...
class ScatterPlot3D(CommonTools):
    ...
    def scatterplot3D(self):
        ...
        x_y_z_cat = []
        x_y_z_cat.extend([list(a)
                          for a in zip(self.x, self.y, self.z, self.cat)])
        ...
        for i in range(len(categories)):
            for itr in range(total):
                if categories[i] == x_y_z_cat[itr][-1]:
                    ...
                    bpy.context.active_object.name = "Scatter No: (" + str(self.x[itr]) + ", " + str(
                        self.y[itr]) + ", " + str(self.z[itr]) + "), Cat :" + str(categories[i])

                    # The material will be created and applied.
                    activeObject = bpy.context.active_object
                    material = PrincipleMaterial(
                        "ScatterMaterial :" + str(categories[i]), self.scatter_material[i][1])
